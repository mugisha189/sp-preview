...
function PhoneInput({ className, onChange, ...props }: React.ComponentProps<typeof RPNInput.default> & PhoneInputProps) {
  return (
    <RPNInput.default
      className={cn(
        'flex rounded-md focus-within:ring-1',
        className,
        props.value && !RPNInput.isValidPhoneNumber((props.value ?? '') as string) ? 'ring-red-500' : 'ring-primary-700',
      )}
    ...
    <Popover modal>
      <PopoverTrigger asChild>
        <Button
          type="button"
          variant="outline"
          className="flex gap-1 rounded-e-none rounded-s-md border-r-0 px-3 focus:z-10 h-auto"
          disabled={disabled}
        >
      ...
      <PopoverContent className="p-2 sm:p-0 w-[var(--radix-popover-trigger-width)] min-w-64" align="start" sideOffset={5}>
        <Command>
          <CommandInput
            value={searchValue}
            onValueChange={(value) => {
...
const FlagComponent = ({ country, countryName }: RPNInput.FlagProps) => {
  ...
  return (
    <span className="flex h-4 w-6 overflow-hidden rounded-sm [&_svg]:size-full">{Flag && <Flag title={countryName} />}</span>
  );
};
